...
int main()
{
	...
	while(true)
	{
		...
		if(recieved(rci) && rci.dataSize == SubmarinePacket::serializedSize())
		{
			...
			horizon.sendAngles(-static_cast<double>(Saxises[4])/std::numeric_limits<int16_t>::max()*360, static_cast<double>(Saxises[3])/std::numeric_limits<int16_t>::max()*360, inPacket.depth);
		}
		msg.clear();
